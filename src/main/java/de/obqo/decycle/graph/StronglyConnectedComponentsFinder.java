...
public class StronglyConnectedComponentsFinder {
     ...
     * (SCC) of the given directed {@code graph}.
     * For a cycle-free graph this method returns an empty set.
     *
    ...
    /**
     * The {@code TarjansAlgorithm} class represents a data type for
     * determining the strong components in a directed graph.
     * With {@link #getMultiNodeComponents()} it will return only
     * those components that contain more than one node.
     *
     * <p>
     * This implementation uses Tarjan's algorithm.
     * The constructor takes time proportional to <em>N</em> + <em>E</em>
     * (in the worst case),
     * where <em>N</em> is the number of nodes (or vertices) and <em>E</em> is the number of edges.
